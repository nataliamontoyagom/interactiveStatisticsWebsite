...
::: {.callout-tip collapse="true"}
## Probar funciones predefinidas para el manejo de texto
...
::: {.callout-tip collapse="true"}
## Copia los ejemplos que más te gusten de la tabla y pruébalos tú mismo aquí
...
```{webr-r}
datos <- read.csv2("https://raw.githubusercontent.com/nataliamontoyagom/interactiveStatisticsWebsite/main/futbol.csv")
```
...
Para poder guardar esta modificación, se utiliza el siguiente comando en RStudio:

```{webr-r}
write.csv2(datos,"futbolcambiado.csv")
```

Podrás comprobar en la sección `Files` de RStudio que aparece el nuevo archivo de datos en la carpeta de trabajo.
